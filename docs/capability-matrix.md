...
| MCP operations              | ❌ | ❌ (MCP disabled for the SDK profile) | ⚠️ config inspect/list only, via CLI argv, no credential exposure | ⚠️ same subset; control plane implemented, but live tool/resource/prompt catalogs, provider-owned OAuth, and direct invocation remain unavailable for either provider today |
| Components (skills/plugins/hooks) | ❌ | ❌ | ⚠️ read-only discovery only; execution blocked until workspace trust is granted; no management/invocation | ⚠️ same read-only scope |
| Child-agent (subagent) events | ❌ | ❌ | ⚠️ storage/routes exist, but production provider events do not currently populate the graph; graph controls remain unsupported for either provider | ⚠️ same |
