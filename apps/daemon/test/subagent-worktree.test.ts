...
describe('owned worktree manager', () => {
  it('previews ignored/include risks and never cleans a dirty owned worktree', async () => {
    ...
    await expect(manager.create({ cwd: repo, name: 'invalid', ref: '--help', confirmIncludeCopy: true })).rejects.toMatchObject({ code: 'invalid_ref' } satisfies Partial<WorktreeManagerError>);
    ...
    expect(JSON.parse(await readFile(join(base, 'worktrees.json'), 'utf8')).worktrees).toContainEqual(expect.objectContaining({ id: created.id, status: 'ready' }));
    const recovered = new OwnedWorktreeManager(owned, join(base, 'worktrees.json'));
    await recovered.load();
    expect(await recovered.list()).toContainEqual(expect.objectContaining({ id: created.id, status: 'ready' }));
    await writeFile(join(target, '.env'), 'SECRET=changed-after-copy');
    expect((await run('git', ['status', '--porcelain=v1', '--untracked-files=all'], { cwd: target })).stdout).not.toContain('.env');
    await expect(recovered.cleanup(created.id)).rejects.toMatchObject({ code: 'worktree_dirty' } satisfies Partial<WorktreeManagerError>);
