...
- **Execution**: `claude -p <prompt> --output-format stream-json --verbose`, plus either
  ...
  Passing our own UUID as `--session-id` means the daemon's session id and Claude's own session id
  are the same value from the start, instead of needing to reconcile two ids after the fact.
...
- **Execution**: `codex exec <prompt> --json --skip-git-repo-check`, or
  ...
  prior thread. `--skip-git-repo-check` is required because a session's working directory is
  whatever the user picked, not necessarily a git repository.
...
Say you want to add `GeminiProvider`. You should not need to touch the daemon's routes or the
desktop UI at all:
...
3. **Write tests.** Unit-test the parser against fixture JSON (see `test/codex-parser.test.ts` for
   the shape), and an integration test that spawns a small `node` fixture script standing in for
   the real CLI (see `test/run-session.test.ts` / `test/fixtures/*.mjs`) so CI never needs a real
   Gemini account.
4. **Register it.** Add `new GeminiProvider(logger)` to `buildProviderRegistry()` in
   `apps/daemon/src/providers.ts`.

That's the whole surface. `GET /providers` picks it up automatically (it iterates the registry),
and the desktop UI's provider `<select>` only needs a new `<option>` — its event rendering already
works for any provider because it only ever switches on `AgentEvent.type`.
