...
| MCP operations              | ⚠️ config inspect/list only, via CLI argv, no credential exposure | ❌ (MCP disabled for the SDK profile: session options hardcode an empty server list regardless of what's configured) | ⚠️ same config-only scope as Claude CLI | ⚠️ same subset; control plane implemented, but live tool/resource/prompt catalogs, provider-owned OAuth, and direct invocation remain unavailable for either provider today |
| Components (skills/plugins/hooks) | ⚠️ read-only discovery only (skills, plugins, and -- Claude only -- commands, agents, hooks); no management/invocation for any provider, trusted or not | ❌ (component loading is not wired into SDK session dispatch) | ⚠️ read-only discovery only (skills, plugins); no management/invocation | ⚠️ same read-only scope |
| Child-agent (subagent) events | ⚠️ storage/routes exist (provider-agnostic; not wired to either provider's adapter code), but no adapter populates the graph with a real event | ⚠️ same | ⚠️ same | ⚠️ same |
| Attachments                 | ❌ | ❌ | ❌ session creation has no attachment field; nothing is dispatched to provider execution | ❌ same |
| Structured output           | ❌ | ❌ | ❌ session creation has no output-schema field; validation exists client-side only, never dispatched to a provider | ❌ same |
