/**
 * Small callback-to-async-iterator bridge with explicit failure propagation and an optional
 * byte-size bound -- originally scoped to Codex app-server's raw tool output side channel (see
 * codex/app-server/channel.ts, which re-exports this for its existing call sites), promoted here
 * (issue #132) so every provider adapter's own raw-output channel can reuse the same,
 * already-reviewed backpressure logic instead of reimplementing it per provider.
 */
